		...
		else
		{
			...
			if($numberPhoto && $dataMultiTemp)
			{
				for($i=0;$i<count($dataMultiTemp);$i++)
				{
					...
					if(isset($config['photo']['man_photo'][$type]['images_photo']) && isset($config['photo']['man_photo'][$type]['video_photo']) && $config['photo']['man_photo'][$type]['video_photo'] == true && $config['photo']['man_photo'][$type]['images_photo'] == true)
					{
						if(isset($_FILES["file".$i]))
						{
							...
							if(isset($dataMulti['link_video']) && $dataMulti['link_video'] != ''){
								if(!$d->insert('photo',$dataMulti)) $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=photo&act=man_photo&type=".$type."&p=".$curPage, false);
							}
							
						}
					}
					elseif(isset($config['photo']['man_photo'][$type]['images_photo']) && $config['photo']['man_photo'][$type]['images_photo'] == true)
					{
						if(isset($_FILES["file".$i]))
						{
							$file_name = $func->uploadName($_FILES["file".$i]["name"]);
