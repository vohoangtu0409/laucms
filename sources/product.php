<?php  
	...
	{
		/* Lấy sản phẩm detail */
		$row_detail = $d->rawQueryOne("select type, id, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, noidung$lang, masp, luotxem, id_brand, id_mau, id_size, id_list, id_cat, id_item, id_sub, id_tags, photo, options, giacu,giakm, giamoi, gia from #_product where id = ? and type = ? and hienthi > 0 limit 0,1",array($id,$type));
		...
		$sql = "select photo, ten$lang, tenkhongdauvi, tenkhongdauen, giamoi, gia, giakm,giacu, id from #_product where $where order by stt,id desc $limit";
