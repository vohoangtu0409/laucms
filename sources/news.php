<?php  
	...
	{
		/* Lấy bài viết detail */
		$row_detail = $d->rawQueryOne("select id, luotxem, ngaytao, id_list, id_cat, id_item, id_sub, type, ten$lang,id_tags, tenkhongdauvi, tenkhongdauen, noidung$lang, photo from #_news where id = ? and type = ? and hienthi > 0 limit 0,1",array($id,$type));
		...
		if($row_detail['id_tags']) $news_tags = $d->rawQuery("select id, ten$lang, tenkhongdauvi, tenkhongdauen from #_tags where id in (".$row_detail['id_tags'].") and type='".$type."'");
		...
		$sql = "select id,taptin, ten$lang, tenkhongdauvi, tenkhongdauen, photo, ngaytao, mota$lang from #_news where $where order by stt,id desc $limit";
